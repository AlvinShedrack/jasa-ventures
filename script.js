...
function ensurePagination(table,key,getPage,setPage,render,getTotal){
  ...
  if(!c){
    ...
    c.id=key+'Pagination';
    c.style.cssText='display:flex;align-items:center;justify-content:center;gap:10px;flex-wrap:wrap;padding:12px 4px;font-size:13px;';
    c.innerHTML=`<button type="button" id="${key}PrevPage" class="secondary-btn">Previous</button><span id="${key}PageInfo"></span><button type="button" id="${key}NextPage" class="secondary-btn">Next</button>`;

    const w=table.closest('.table-wrapper');
    if(w) w.insertAdjacentElement('afterend',c);
    else table.insertAdjacentElement('afterend',c);
  }

  const prev=document.getElementById(key+'PrevPage');
  const next=document.getElementById(key+'NextPage');

  // Rebind on every render so the buttons always use the latest filtered records.
  ...
  if(next) next.onclick=()=>{
    const totalPages=Math.max(1,Math.ceil(getTotal()/TABLE_PAGE_SIZE));
